...
class LoginController(ws: WSClient, fbConfig: LoginController.FacebookConfig, configSecret: String)
  ...
  val loginPage = Action { implicit request =>
    ...
    val fbUrl =
      s"https://www.facebook.com/v2.8/dialog/oauth?client_id=$appId&redirect_uri=$redirectUri"
    SeeOther(fbUrl)
  }
  ...
  def loginCallback = Action.async { implicit request =>
    ...
    val tokenUrl =
      s"https://graph.facebook.com/v2.8/oauth/access_token?client_id=$appId&redirect_uri=$redirectUri&client_secret=$appSecret&code=$code"
